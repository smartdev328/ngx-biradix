...
define([
    "app",
     ...
     app.controller("editReportController", ["$scope", "$uibModalInstance", "report", "ngProgress", "toastr", "$dialog", "$saveReportService", function($scope, $uibModalInstance, report, ngProgress, toastr, $dialog, $saveReportService) {
                ...
                $dialog.confirm("Are you sure you want to delete <b>" + report.name +"</b>?", function() {
                    $saveReportService.remove(report._id).then(function(response) {
                            ...
                            if (response.data.errors) {
                                ...
                                toastr.success("<B>" + report.name + "</B> custom report template deleted successfully.");
