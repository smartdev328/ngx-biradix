...
define([
    ...
    app.controller('reportingController', ['$scope','$rootScope','$location','$propertyService','$auditService', 'ngProgress', '$progressService','$cookies','$window','toastr','$reportingService','$stateParams','$urlService','$uibModal','$saveReportService','$cookieSettingsService','$q',"$exportService"
        , function ($scope,$rootScope,$location,$propertyService,$auditService,ngProgress,$progressService,$cookies,$window,toastr,$reportingService,$stateParams,$urlService,$uibModal,$saveReportService,$cookieSettingsService,$q,$exportService) {
        ...
        var me = $rootScope.$watch("me", function(x) {
            if ($rootScope.me) {
                ...
                $scope.reportItems.push({id: "reputation", name: "Reputation", selected: false, group: "Individual Reports", type: "single", tooltip: "<b>Reputation Report</b> - <i>Report offers a quick and easy way to compare the review/reputation scores of the subject property and its comps on popular review sites such as Yelp and Google.</i>"});
        ...
        $scope.checkReportRunning = function() {
            ...
            if (seconds > 90) {
                ...
                $scope.reportLoading = false;
                window.renderable = true;
                rg4js('send', new Error("Report took longer then 90 seconds"));
                return;
            }
        ...
        $scope.reportsChanged = function(load, callback) {
            ...
            $scope.excel_rankings = function(settings) {
                ngProgress.start();
        ...
        $scope.configurePropertyStatusOptions = function() {
            ...
            $scope.configureCustomPortfolioOptions = function() {
                ...
                $scope.temp.customPortfolioSortItems = [
                    ...
                    {id: "nersqft0", name: "NER/Sqft - Studios"},
                    {id: "nersqft1", name: "NER/Sqft - 1 Bds."},
                    {id: "nersqft2", name: "NER/Sqft - 2 Bds."},
                    {id: "nersqft3", name: "NER/Sqft - 3 Bds."},
        ...
        $scope.singlePerspective = function(strPerspectives, strSelectedPerspective, settingsPerspective) {
            if ($scope.selected.Property) {
                $scope.temp[strPerspectives] = [{value: "", text: "All Data"}];
                ($scope.selected.Property.perspectives || []).forEach(function(p) {
                    $scope.temp[strPerspectives].push({value: p.id, text: p.name});
                });

                $scope.temp[strSelectedPerspective] = _.find($scope.temp[strPerspectives], function(x) {
                    return x.value.toString() === settingsPerspective;
                });

                if (!$scope.temp[strSelectedPerspective]) {
                    $scope.temp[strSelectedPerspective] = $scope.temp[strPerspectives][0];
                }
            }
        };
