...
define([
    ...
    app.controller('reportingController', ['$scope','$rootScope','$location','$propertyService','$auditService', 'ngProgress', '$progressService','$cookies','$window','toastr','$reportingService','$stateParams','$urlService','$uibModal','$saveReportService','$cookieSettingsService','$q'
        ...
        var me = $rootScope.$watch("me", function(x) {
            if ($rootScope.me) {
                $scope.reportItems.push({id: "community_amenities", name: "Community Amenities", selected:false, group: "Individual Reports", type:"single", tooltip: "<b>Community Amenities Report</b><Br>Report highlights all Community Amenities for the subject property and each comp property. <Br><Br><i>Additional info:<Br>Community Amenities are listed by category for subject property and each comp, making it easier to compare what each property has to offer. The Total Amenity score adds all amenities for each property. </i>"});
                $scope.reportItems.push({id: "concession", name: "Concessions", selected:$stateParams.property == "2", group: "Individual Reports", type:"single", tooltip: "<b>Concessions Report</b><Br>Report compares concessions over a customizable period of time for the subject property and comps. It includes Market Rent, One-Time Concessions, Recurring Concessions, Total Concessions, NER, and NER vs. Weighted Avg. Comp NER<Br><Br><i>Additional info:<Br>Report analyzes market rent and concessions for subject property and each comp. This allows the user to understand better the dynamics of submarket concessions and which comp is offering One-Time Concessions, Recurring Concessions, or both. The report is especially useful in a market with different types of concessions and frequent price changes. </i>"});
                $scope.reportItems.push({id: "fees_deposits", name: "Fees & Deposits", selected:false, group: "Individual Reports", type:"single", tooltip: "<b>Fees and Deposits Report</b><Br>Report includes different fees and deposits for each property including: Administration Fee, Application Fee, Lease Terms, Pet Deposit (Refundable & Non-Refundable), Security Deposit, Pet Rent, and Short Term Rental Premium <Br><Br><i>Additional info:<Br>Report allows users to compare and contrast different fees and deposits for subject property and each comp, as well as compare pet rent and short term lease premiums. </i>"});
                $scope.reportItems.push({id: "location_amenities", name: "Location Amenities", selected:false, group: "Individual Reports", type:"single", tooltip: "<b>Location Amenities Report</b><Br>Report highlights all Location Amenities for the subject property and each comp property. <Br><Br><i>Additional info:<Br>Location Amenities are listed by category for subject property and each comp, making it easier to compare what each property has to offer. The Total Amenity score adds all amenities for each property. </i>"});
                $scope.reportItems.push({id: "property_report", name: "Market Survey Summary", selected:$stateParams.property == "1", group: "Individual Reports", type:"single", tooltip: "<b>Market Survey Summary</b><Br>Report includes historic trends for key performance indicators including NER or NER/SQFT, Occupancy and Leased percentage in addition to property summaries for the subject property and each comps. The report allows users to benchmark the subject property vs. one or more comps and provides the ability to drill down by NER or NER/SQFT to the floor plan (bedroom) level for a customizable time-frame (ex: last 90 days.)<Br><Br><i>Additional info:<Br>This is the most comprehensive weekly report that, by default, includes the subject property and all comps. It can be customized many different ways. Including ability to exclude any comp properties, drill down by specific floor plan (ex: compare only 1 bedrooms), or select a specific time frame to run the report (ex: last 90 days, or 45 days). See User Guide for more details on customizing reports. </i>"});
                $scope.reportItems.push({id: "property_rankings_summary", name: "Floor Plan Comparison", selected:$stateParams.property == "3", group: "Individual Reports", type:"single", tooltip: "<b>Floor Plan Comparison</b><Br>Report lets the user quickly sort, rank and analyze the current performance of the subject property versus each of the comps at the property and floor plan level<Br><Br><i>Additional info:<Br>Report can be customized in many different ways to include different data points (such as Rent, Concessions, NER, SQFT, etc.). Report can be used to benchmark subject property performance by sorting any of the data points, such as NER/SQFT, NER, SQFT and comparing against individual comps, and weighted averages for the entire comp set, or by floor plan. </i>"});
                $scope.reportItems.push({id: "property_rankings", name: "Floor Plan Comparison (detailed)", selected:$stateParams.property == "4", group: "Individual Reports", type:"single", tooltip: "<b>Floor Plan Comparison (Detailed)</b><Br>Similar to Floor Plan Comparison report, the \"detailed\" version includes the subject property and each comp and a detailed breakdown by individual floor plans as opposed to grouping by # of bedrooms.<Br><Br><i>Additional info:<Br>Report includes breakdown of Unit Types and can be customized in many different ways to include different data points (such as Rent, Concessions, NER, SQFT, etc.). Report can be used to benchmark subject property performance by sorting any of the data points, such as NER/SQFT, NER, SQFT and comparing against individual comps, and weighted averages for the entire comp set, or by floor plan.</i>"});
                    ...
                    $scope.reportItems.push({
                        ...
                        type: "single",
                        tooltip: "<b>Trend Analysis</b><Br>Report allows comparison of data (Rent, NER, NER/SQFT, Occupancy, Leased %, Traffic & Leases, etc.) over two different time periods (ex: last 30 days vs. previous 30 days), as well as benchmarking against competitor data.<Br><Br><i>Additional info:<Br>The report enables the user to quickly compare their property performance over any two different time frames, such as Quarter-over-Quarter, and compare it against comp performance over the same time frame. The report is very useful when analyzing seasonality, leasing velocity and occupancy, NER, and any other data points. </i>",
                    });
                $scope.reportItems.push({id: "property_status", name: "Property Status", selected: false, group: "Portfolio Reports", type:"multiple", tooltip: "<b>Property Status</b><Br>Report is intended to give a focused summary on the current state of your portfolio. It can be modified to include data points such as Traffic & Leases, NER and NER/SQFT, as well as NER changes vs. Last Week, Last Month, or vs. Last Year, as well as how each property is performing vs. weighted avg. for all properties.<Br><Br><i>Additional info:<Br>The report is highly customizable and can include many different data points for one or more subject properties and all comps. it allows the user to monitor portfolio performance vs. comp properties by comparing week-over-week, month-over-month, or year-over-year performance. It also makes it easy to compare how much above or below each property is vs. weighted avg. for all properties included. The report can also automatically be emailed to any user, including corporate users and ownership/investor groups. </i>"});
        ...
        $scope.configurePropertyStatusOptions = function() {
            ...
            $scope.configureCustomPortfolioOptions = function() {
                ...
                $scope.temp.showCustomPortfolioItems = [
                    ...
                    {id: "nersqft", name: "NER / Sqft", selected: $scope.liveSettings.customPortfolio.show.nersqft, tooltip: "<b>Net Effective Rent / Sqft</b><br><i>Net Effective Rent per Square Foot (NER divided by Sqft)</i>"},
