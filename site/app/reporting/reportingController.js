...
define([
    ...
    app.controller('reportingController', ['$scope','$rootScope','$location','$propertyService','$auditService', 'ngProgress', '$progressService','$cookies','$window','toastr','$reportingService','$stateParams','$urlService','$uibModal','$saveReportService','$cookieSettingsService','$q'
        ...
        var me = $rootScope.$watch("me", function(x) {
            ...
            if ($rootScope.me) {
                console.log($rootScope.me, "Inside");
                ...
                $scope.reportItems.push({id: "fees_deposits", name: "Fees & Deposits", selected:false, group: "Individual Reports", type:"single", tooltip: "<b>Fees and Deposits Report</b><Br><i>Report includes different fees and deposits for each property including: Administration Fee, Application Fee, Lease Terms, Pet Deposit (Refundable & Non-Refundable), Security Deposit, Pet Rent, and Short Term Rental Premium</i>"});
                $scope.reportItems.push({id: "location_amenities", name: "Location Amenities", selected:false, group: "Individual Reports", type:"single", tooltip: "<b>Location Amenities Report</b><Br><i>Report highlights all Location Amenities for the subject property and each comp property.</i>"});
                $scope.reportItems.push({id: "property_report", name: "Market Survey Summary", selected:$stateParams.property == "1", group: "Individual Reports", type:"single", tooltip: "<b>Market Survey Summary</b><Br><i>Report includes historic trends for key performance indicators including NER or NER/SQFT, Occupancy and Leased percentage in addition to property summaries for the subject property and each comps. The report allows users to benchmark the subject property vs. one or more comps and provides the ability to drill down by NER or NER/SQFT to the floor plan (bedroom) level for a customizable time-frame (ex: last 90 days.)</i>"});
                $scope.reportItems.push({id: "property_rankings_summary", name: "Floor Plan Comparison", selected:$stateParams.property == "3", group: "Individual Reports", type:"single", tooltip: "<b>Floor Plan Comparison</b><Br><i>Report lets the user quickly sort, rank and analyze the current performance of the subject property versus each of the comps at the property and floor plan level</i>"});
                $scope.reportItems.push({id: "property_rankings", name: "Floor Plan Comparison (detailed)", selected:$stateParams.property == "4", group: "Individual Reports", type:"single", tooltip: "<b>Floor Plan Comparison (Detailed)</b><Br><i>Similar to Floor Plan Comparison report, the \"detailed\" version includes the subject property and each comp and a detailed breakdown by individual floor plans as opposed to grouping by # of bedrooms.</i>"});
                    ...
                    $scope.reportItems.push({
                        ...
                        type: "single",
                        tooltip: "<b>Trend Analysis</b><Br><i>Report allows comparison of data (Rent, NER, NER/SQFT, Occupancy, Leased %, Traffic & Leases, etc.) over two different time periods (ex: last 30 days vs. previous 30 days), as well as benchmarking against competitor data.</i>",
                    });
                $scope.reportItems.push({id: "property_status", name: "Property Status", selected: false, group: "Portfolio Reports", type:"multiple", tooltip: "<b>Property Status</b><Br><i>Report is intended to give a focused summary on the current state of your portfolio. It can be modified to include data points such as Traffic & Leases, NER and NER/SQFT, as well as NER changes vs. Last Week, Last Month, or vs. Last Year, as well as how each property is performing vs. weighted avg. for all properties.</i>"});
                $scope.reportItems.push({id: "custom_portfolio", name: "Customizable Property Data", selected: false, group: "Portfolio Reports", type:"multiple", tooltip: "<b>Customizable Property Data</b><Br><i>Report offers customizable solution to ad hoc building report tables with any property data available in BI:Radix. Allows modifying column order, setting sort preference, and exporting to Excel.</i>"});
