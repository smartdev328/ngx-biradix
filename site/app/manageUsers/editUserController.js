...
define([
    ...
    app.controller("editUserController", ["$scope", "$uibModalInstance", "userId", "$userService", "ngProgress", "$propertyService", "$propertyUsersService", "toastr", "$rootScope",
        ...
        $scope.getOrg = function(role) {
            var org = _.find($scope.orgs, function(o) {
                return role && role.orgid && o.id.toString() === role.orgid.toString();
            });
        ...
        $scope.addRole = function() {
            $scope.user.roles.push({selectedOrg: $scope.getOrg($scope.roles[0]), selectedRole: $scope.roles[0], propertyids: []});
        };
            ...
            $userService.getRolesToAssign().then(function(response) {
                    ...
                    if (userId) {
                        $scope.user.roles.forEach(function(r) {
                            ...
                            r.selectedOrg = $scope.getOrg(r);
                        });
                    } else {
                        $scope.user.roles[0].selectedRole = $scope.roles[0];
                        $scope.user.roles[0].selectedOrg = $scope.getOrg($scope.roles[0]);
                    }
        ...
        $scope.getProps = function(role, first) {
            ...
            if (!role.selectedRole || !role.selectedRole._id || role.selectedRole.isadmin || role.selectedRole.tags.indexOf("CM") > -1) {
                role.properties = [];
                if (!first) {
                    $scope.loading = false;
                }
        ...
        $scope.allowedRoles = function(index){
            ...
            $scope.allowedOrgs = function(org) {
                return function(item) {
                    if (item._id === "") {
                        return true;
                    }
                    ...
                    return (item.orgid || "").toString() === org.id.toString();
                };
            };
